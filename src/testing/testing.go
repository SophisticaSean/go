...
type common struct {
	...
	failureMsg  []byte               // Test or benchmark failure message.
...
func (c *common) FailureMsg() string {
	return string(c.failureMsg)
}
...
func (c *common) logDepth(s string, depth int) {
	...
	if c.done {
		...
		for parent := c.parent; parent != nil; parent = parent.parent {
			...
			if !parent.done {
				decoratedLog := parent.decorate(s, depth+1)
				parent.output = append(parent.output, decoratedLog...)
				parent.failureMsg = append(parent.failureMsg, decoratedLog...)
				return
			}
		}
		panic("Log in goroutine after " + c.name + " has completed: " + s)
	} else {
		decoratedLog := c.decorate(s, depth+1)
		c.failureMsg = append(c.failureMsg, decoratedLog...)
		if c.chatty != nil {
			if c.bench {
				// Benchmarks don't print === CONT, so we should skip the test
				// printer and just print straight to stdout.
				fmt.Print(decoratedLog)
			} else {
				c.chatty.Printf(c.name, "%s", decoratedLog)
			}

			return
		}
		c.output = append(c.output, decoratedLog...)
	}
}
...
func (f matchStringOnly) CoordinateFuzzing(
	time.Duration,
	int64,
	time.Duration,
	int64,
...
func (m *M) Run() (code int) {
	...
	if !*isFuzzWorker {
		...
		if !testOk || !exampleOk || !fuzzTargetsOk || !runBenchmarks(m.deps.ImportPath(), m.deps.MatchString, m.benchmarks) ||
			race.Errors() > 0 {
